...
class AudioDecoderFrame(ttk.Frame):
    """独立的音频解码与信号分析组件类"""
    ...
    def _build_ui(self):
        ...
        self.filename_var = tk.StringVar(value="未加载文件")
        ttk.Label(
            control_frame,
        ...
        ttk.Label(window_frame, text="数据起始(s):").pack(side=tk.LEFT)
        self.data_start_var = tk.StringVar(value="0.0")
        self.data_start_entry = ttk.Entry(
            window_frame, textvariable=self.data_start_var, width=8
        )
        self.data_start_entry.pack(side=tk.LEFT, padx=2)

        ttk.Label(window_frame, text="窗口(s):").pack(side=tk.LEFT)
        self.data_window_var = tk.StringVar(value="0.1")
        self.data_window_entry = ttk.Entry(
            window_frame, textvariable=self.data_window_var, width=8
        )
        self.data_window_entry.pack(side=tk.LEFT, padx=2)

        self.data_start_entry.bind(
            "<Return>", lambda e: self.on_window_params_changed()
        )
        self.data_start_entry.bind(
            "<FocusOut>", lambda e: self.on_window_params_changed()
        )
        self.data_window_entry.bind(
            "<Return>", lambda e: self.on_window_params_changed()
        )
        self.data_window_entry.bind(
            "<FocusOut>", lambda e: self.on_window_params_changed()
        ...
        ttk.Label(env_frame, text="包络类型:").pack(side=tk.LEFT)
        self.env_var = tk.StringVar(value="RC包络")
        ...
        env_combo.bind("<<ComboboxSelected>>", self.on_envelope_type_changed)

        # 判决模式选择
        decision_frame = ttk.Frame(control_frame)
        decision_frame.pack(fill=tk.X, pady=2)
        ttk.Label(decision_frame, text="0/1判断方法:").pack(side=tk.LEFT)
        self.decision_var = tk.StringVar(value="宽度阈值")
        ...
        decision_combo.pack(side=tk.LEFT, padx=5)
        decision_combo.bind(
            "<<ComboboxSelected>>", self.on_decision_method_changed
        )
        ...
        self.params = [
            ...
            ("最小宽度(ms)", "low_bond", 0, 3.0, 0.2, 0.01),
            ("0/1阈值(ms)", "bit_thresh", 0, 5.0, 2.4, 0.01),
            ("最大宽度(ms)", "high_bond", 0.5, 10.0, 4.0, 0.01),
            ("合并间隔(ms)", "min_gap", 0.1, 5, 0.4, 0.1),
        ]
        self.slider_attrs = {}
        self.slider_vars = {}
        self.entry_vars = {}
        ...
        for idx, (label, attr, vmin, vmax, vinit, step) in enumerate(
            self.params
        ):
            frame = ttk.Frame(self.params_frame)
            frame.pack(fill=tk.X, pady=1)
            self.param_frames[attr] = frame
            ...
            scale.pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)
            self.scales[attr] = scale

            entry_var = tk.StringVar(
                value=f"{vinit:.3f}" if attr != "min_gap" else f"{vinit:.1f}"
            )
            self.entry_vars[attr] = entry_var
            ...
            scale.bind(
                "<ButtonRelease-1>",
                lambda e, a=attr: self.on_slider_release(a),
            )
            entry.bind("<Return>", lambda e, a=attr: self.on_entry_enter(a))
        ...
        ttk.Button(
            btn_frame, text="载入 WAV 音频", command=self.load_new_file
        ...
        ttk.Button(
            hist_frame, text="刷新直方图", command=self.refresh_histogram
        ).pack(pady=2)
    ...
    # ---------- 事件处理与图形控制 ----------
    def on_envelope_type_changed(self, event=None):
        self.envelope_type = "rc" if self.env_var.get() == "RC包络" else "peak"
        self.hist_data_valid = False
        if self.raw is not None:
            self.update_all_and_plot()
            self.update_histogram()

    def on_decision_method_changed(self, event=None):
        method = self.decision_var.get()
        self.bit_decision_method = "width" if method == "宽度阈值" else "area"
        self.hist_data_valid = False
        if self.bit_decision_method == "width":
            self._update_slider_for_width()
            self.ax_area.set_ylabel("脉冲宽度 (ms)")
            self.ax_area.set_title("脉冲宽度随时间分布")
        else:
    ...
    def _update_slider_for_width(self):
        self._apply_slider_config(
            "low_bond", "最小宽度(ms)", 0, 3.0, 0.2, 0.01
        )
        self._apply_slider_config(
            "bit_thresh", "0/1阈值(ms)", 0, 5.0, 2.4, 0.01
        )
        self._apply_slider_config(
            "high_bond", "最大宽度(ms)", 0.5, 10.0, 4.0, 0.01
        )
        self.low_bond = 0.2
        self.bit_thresh = 2.4
        self.high_bond = 4.0
        self.area_thresh = self.bit_thresh
        self.slider_vars["bit_thresh"].set(2.4)
        self.entry_vars["bit_thresh"].set("2.400")

    def _update_slider_for_area(self):
        self._apply_slider_config(
            "low_bond", "最小面积(幅度·ms)", 0, 1.0, 0.5, 0.01
        )
        self._apply_slider_config(
            "bit_thresh", "面积阈值(幅度·ms)", 0, 3.0, 1.2, 0.01
        )
        self._apply_slider_config(
            "high_bond", "最大面积(幅度·ms)", 0.5, 4.0, 2.0, 0.01
        )
        self.low_bond = 0.5
        self.bit_thresh = 1.2
        self.high_bond = 2.0
        self.area_thresh = self.bit_thresh
        self.slider_vars["bit_thresh"].set(1.2)
        self.entry_vars["bit_thresh"].set("1.200")

    def _apply_slider_config(self, attr, label, vmin, vmax, vinit, step):
        frame = self.param_frames[attr]
        label_widget = frame.winfo_children()[0]
        label_widget.config(text=label)
        scale = self.scales[attr]
        scale.config(from_=vmin, to=vmax)
        self.entry_vars[attr].set(f"{vinit:.3f}")
        self.slider_vars[attr].set(vinit)
    ...
    def on_window_params_changed(self):
        try:
            start = float(self.data_start_var.get())
            window = float(self.data_window_var.get())
            if window <= 0:
                window = 0.1
    ...
    def on_slider_release(self, attr):
        val = self.slider_vars[attr].get()
        if attr == "min_gap":
            self.entry_vars[attr].set(f"{val:.1f}")
            setattr(self, attr, val / 1000.0)
        else:
            self.entry_vars[attr].set(f"{val:.3f}")
    ...
    def on_entry_enter(self, attr):
        try:
            val = float(self.entry_vars[attr].get())
            scale = self.scales[attr]
            if val < scale["from"]:
                val = scale["from"]
            elif val > scale["to"]:
    ...
    def load_file(self, filename):
        ...
        self.data_start_var.set("0.0")
        self.data_window_var.set(f"{self.data_window:.3f}")
        self.hist_data_valid = False
        self.update_all_and_plot()
        self.update_histogram()
    ...
    def rc_filter(self, data, alpha):
        ...
        for i in range(len(data)):
            filtered[i] = alpha * data[i] + (
                1 - alpha
            ) * (filtered[i - 1] if i > 0 else 0)
        return filtered
    ...
    def update_all_and_plot(self):
        ...
        try:
            ...
            if self.bit_decision_method == "width":
                valid_mask = (raw_widths > self.low_bond) & (
                    raw_widths < self.high_bond
                )
            else:
                valid_mask = (raw_areas > self.low_bond) & (
                    raw_areas < self.high_bond
                )
            ...
            if self.bit_decision_method == "width":
                ...
                valid_bits = (
                    bits[valid_mask] if len(raw_widths) > 0 else np.array([])
                )
            else:
                ...
                valid_bits = (
                    bits[valid_mask] if len(raw_areas) > 0 else np.array([])
                )
    ...
    def plot_all(self):
        ...
        if len(self.bit_wave) > 0:
            ...
            self.ax_orig.plot(
                t_slice, bit_slice, "g-", lw=1.5, alpha=0.7, label="解码 0/1"
            )
        ...
        self.ax_rect.plot(
            t_slice, rect_slice, "c-", lw=0.5, alpha=0.3, label="整流"
        )
        ...
        if self.bit_decision_method == "width":
            ...
            if xlim_cur[0] != xlim_cur[1]:
                self.ax_area.plot(
                    xlim_cur,
                    [self.low_bond, self.low_bond],
                    "b--",
                    label="最小宽度",
                )
                self.ax_area.plot(
                    xlim_cur,
                    [self.bit_thresh, self.bit_thresh],
                    "r--",
                    label="0/1阈值",
                )
                self.ax_area.plot(
                    xlim_cur,
                    [self.high_bond, self.high_bond],
                    "g--",
                    label="最大宽度",
                )
            ymax = (
                max(np.max(data_values), 0.01) if len(data_values) > 0 else 5.0
            )
            ...
            if xlim_cur[0] != xlim_cur[1]:
                ...
                self.ax_area.plot(
                    xlim_cur,
                    [self.area_thresh, self.area_thresh],
                    "r--",
                    label="面积阈值",
                )
                self.ax_area.plot(
        ...
        if len(self.raw_centers) > 0:
            mask = (self.raw_centers >= win_start) & (
                self.raw_centers <= win_end
            )
        ...
        if len(self.bit_centers) > 0:
            mask = (self.bit_centers >= win_start) & (
                self.bit_centers <= win_end
            )
            centers_bit = self.bit_centers[mask]
            if self.bit_decision_method == "width":
                values_bit = (
                    self.widths[mask] if len(self.widths) > 0 else np.array([])
                )
            else:
                values_bit = (
                    self.areas[mask] if len(self.areas) > 0 else np.array([])
                )
    ...
    def plot_histogram(self):
        ...
        if len(data) == 0 or len(filtered) == 0:
            ...
            self.hist_ax.hist(
                filtered, bins=50, alpha=0.7, color="green", edgecolor="black"
            )
            x_max = high * 1.1 if high > 0 else 1.0
            self.hist_ax.set_xlim(left=0, right=x_max)
            self.hist_ax.axvline(
                x=low,
                color="blue",
                linestyle="--",
                linewidth=2,
    ...
    def _auto_export_rom(self, bin_file, show_message=True):
        try:
            ...
            msg = f"无法读取文件：{e}"
            if show_message:
                messagebox.showerror("ROM生成失败", msg)
            return False, msg
        ...
        try:
            ...
            if show_message:
                messagebox.showinfo("ROM生成成功", msg)
            return True, msg
        except Exception as e:
            msg = f"保存 ROM 文件失败：{e}"
            if show_message:
    ...
    def reset(self):
        ...
        self.data_start_var.set("0.0")
        self.data_window_var.set(f"{self.data_window:.3f}")
        self.env_var.set("RC包络")
        self.envelope_type = "rc"
        self.decision_var.set("宽度阈值")
        self.bit_decision_method = "width"
        self._update_slider_for_width()
        self.hist_data_valid = False
        self.update_all_and_plot()
    ...
    def save_bin(self):
        ...
        progress_win.geometry("240x80")
        progress_win.transient(self)
        progress_win.grab_set()
        ttk.Label(progress_win, text="正在计算解码脉冲，请稍候...").pack(
            pady=20
        )
        ...
        if start_idx >= len(self.raw):
            ...
            messagebox.showerror("错误", "数据起始位置已超出文件总长度！")
            return
        ...
        progress_win.destroy()
        if not bits:
            messagebox.showwarning("警告", "未检测到符合条件的有效脉冲数据！")
            return
        ...
        if out_file:
            ...
            rom_success, rom_msg = self._auto_export_rom(
                out_file, show_message=False
            )
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("音频解码器独立测试窗口")
    root.geometry("1100x750")
    app = AudioDecoderFrame(root)
