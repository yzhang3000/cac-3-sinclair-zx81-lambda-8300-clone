...
class SignalAnalyzerFrame(ttk.Frame):
    ...
    def create_widgets(self):
        # ---- Top Frame: 信号源与观测窗口 ----
        top_frame = ttk.LabelFrame(self, text=" 1. 信号源与观测窗口 ", padding=10)
        ...
        # ---- Middle Frame: 参数微调 ----
        mid_frame = ttk.LabelFrame(self, text=" 2. 检波与能量积解码参数实时微调 ", padding=10)
        ...
        self.plot_frame = ttk.LabelFrame(self, text=" 3. 检波波形与能量积分区间分析 ", padding=5)
        ...
        ttk.Label(button_container, text="数据全量分析统计：", foreground="gray", font=("微软雅黑", 10)).pack(anchor="w", pady=(0, 4))
    ...
    def export_bin(self):
        ...
        up_limit_real = self.val_uplimit.get()
        ...
        if not valid_bits:
            ...
            messagebox.showwarning("导出失败", "基于当前参数未提取到任何有效比特，请调低【判定起止阈值】或点一下【刷新片段】。")
            return
